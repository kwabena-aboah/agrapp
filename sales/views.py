...
from .models import Cart, Order, PaymentMethod
from .forms import AddressForm
from products.views import ProductsBaseView, get_default_currency
...
class CheckoutBaseView(ProductsBaseView):
    """
...
class CheckoutReceiptView(CheckoutBaseView):
    ...
    def get(self, request, order_id, receipt_code):
        ...
        if order_confirmed:
            ...
            msg_subject = render_to_string("sales/email/order_confirmation_subject.txt", context)
            context ={
            'order': order, 
            'user': request.user,
            'SITE_NAME': self.get_config('SITE_NAME'), 
            ...
            msg_text = render_to_string("sales/email/order_confirmation.html",context)
