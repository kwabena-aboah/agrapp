...
from agrapp.views import BaseView
...
class ProductsBaseView(BaseView):
    """
    Generic view for all products pages
    """
    style_name = 'products'
    products_template_name = 'products/catalog_base.html'

    def __init__(self, *args, **kwargs):
        super(ProductsBaseView, self).__init__(*args, **kwargs)
        # Loading categories
        ...
        if self.primary_currency is None:
            raise ImproperlyConfigured('No primary currency is defined for agrapp.'
    ...
    def get_context_data(self, **kwargs):
        context = super(ProductsBaseView, self).get_context_data(**kwargs)

        # Setting data context for products base template
        breadcrumbs = ({'name': 'All', 'url': reverse('products_index')},)
        ...
        context['products_template_name'] = self.products_template_name
...
class IndexView(ProductsBaseView):
    """
    Displays list of featured and recently added products
    """
    template_name = 'products/catalog_index.html'
...
class CategoryProductsView(ProductsBaseView):
    """
    ...
    def get(self, request, slug, page_num):
        ...
        products = paginate(Product.category_products(category), self.get_page_size(),
                            page_num, 'products_category', [slug])
...
class ManufacturerProductsView(ProductsBaseView):
    """
    ...
    def get(self, request, slug, page_num):
        ...
        products = paginate(Product.manufacturer_products(manufacturer), self.get_page_size(),
                            page_num, 'products_manufacturer', [slug])
...
class SearchProductsView(ProductsBaseView):
    """
    ...
    def get(self, request, page_num):
        ...
        if form.is_valid():
            ...
            products = paginate(Product.search_advance_products(
                ...
                self.get_page_size(), page_num, 'products_search', qs=query)
...
class ProductDetailView(ProductsBaseView):
    """
...
class ChangeCurrencyView(View):
    ...
    def post(self, request):
        next_url = request.POST['next'] or reverse('products_index')
...
def paginate(query_set, page_size, page_num, url_name, url_args=[], qs=None):
    ...
    if page.has_previous():
        ...
        if previous_page_num == 1:
            ...
            page.previous_url = reverse(url_name, args=(
                url_args + [previous_page_num])) + (qs or '')

    if page.has_next():
        next_page_num = page.next_page_number()
        page.next_url = reverse(url_name, args=(
            url_args + [next_page_num])) + (qs or '')

    page.page_urls = [(page_num, reverse(url_name, args=(url_args + [page_num])) + (qs or ''))
