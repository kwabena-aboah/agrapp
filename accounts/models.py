...
class UserManager(_UserManager):
    ...
    def get_reset_code(self, email):
        ...
        try:
            ...
            raise AgrappError('We can\'t find that email address, sorry!')
    ...
    def reset_password(self, user_id, reset_code, password):
        ...
        if not password:
            raise AgrappError('New password can\'t be blank.')

        try:
            user = self.get(id=user_id)
            if not user.reset_code or user.reset_code != reset_code or user.reset_code_expire < timezone.now():
                raise AgrappError('Password reset code is invalid or expired.')
    ...
    def change_password(self, user, current_password, password):
        ...
        if not user.check_password(current_password):
            raise AgrappError('Your current password is wrong.')
...
class AbstractUser(_AbstractUser):
    """
    An abstract class extending Django authentication user model for Agrapp.
    """
    MALE = 'M'
    FEMALE = 'F'
    GENDERS = ((MALE, 'Male'),
...
class User(AbstractUser):
    """
    Extends Django authentication user model for Agrapp.
    """
