...
from agrapp.exceptions import AgrappError
from agrapp.decorators import anonymous_required
from products.views import ProductsBaseView
...
class LoginView(ProductsBaseView):
    """
    Login view for Agrapp 
    """
    ...
    def post(self, request):
        ...
        if user is not None:
            if user.is_active:
                login(request, user)
                return HttpResponseRedirect(request.POST.get('next', reverse('products_index')))
            else:
...
class RegisterView(ProductsBaseView):
    """
...
class ForgotPasswordView(ProductsBaseView):
    """
    ...
    def post(self, request):
        ...
        if email:
            ...
            try:
                ...
                context = Context({'user': user, 'SITE_NAME': self.get_config(
                    'SITE_NAME'), 'DOMAIN': self.get_config('DOMAIN')})
                msg_subject = get_template(
                    "accounts/email/password_reset_subject.txt").render(context)
                context = Context({'user': user, 'SITE_NAME': self.get_config(
                    'SITE_NAME'), 'DOMAIN': self.get_config('DOMAIN')})
            ...
            except AgrappError as e:
                error = e.message

        return self.get(request, error=error, success=success)


class PasswordResetView(ProductsBaseView):
    """
    Password recovery view
    """
    ...
    def post(self, request, user_id, reset_code):
        ...
        if form.is_valid():
            data = form.cleaned_data
            try:
                User.objects.reset_password(
                    user_id, reset_code, data['password'])
...
class ChangePasswordView(ProductsBaseView):
    """
    Password recovery view
    """
        ...
        if form.is_valid():
            ...
            try:
                User.objects.change_password(
                    request.user, data['current_password'], data['password'])
