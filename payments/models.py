...
class Gateway(models.Model):
    ...
    name = models.CharField(primary_key=True, max_length=10,
                            choices=ALL, help_text='Payment processing gateway.')
    account = models.CharField(
        max_length=100, help_text='Account name of gateway for reference.')
    is_active = models.BooleanField(
        default=False, help_text='Gateway active for customer to buy through it.')
    is_sandbox = models.BooleanField(
        default=False, help_text='Sandbox mode for testing & debugging.')
    accept_credit_card = models.BooleanField(
        default=False, help_text='Process credit card payments.')
        ...
        if self.accept_credit_card:
            ...
            if gateways:
                raise ValidationError(
                    '%s is already configured to accept credit card payments.' % (gateways[0]))
...
class GatewayParam(models.Model):
    ...
    name = models.CharField(
        max_length=250, help_text='Gateway settings parameter name.')
    value = models.CharField(
        max_length=500, help_text='Gateway settings parameter value.')
    updated_on = models.DateTimeField(auto_now=True)
    created_on = models.DateTimeField(auto_now_add=True)
    updated_by = models.CharField(max_length=100)
    created_by = models.CharField(max_length=100)
...
class TransactionParam(models.Model):
    ...
    name = models.CharField(
        max_length=100, help_text='Transaction parameter name.')
    value = models.CharField(
        max_length=250, help_text='Transaction parameter value.')
    created_on = models.DateTimeField(auto_now_add=True)
    created_by = models.CharField(max_length=100)
