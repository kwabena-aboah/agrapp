            ...
            try:
                ...
                if self.id != primary_currency.id:
                    raise ValidationError(
                        '"%s" is already defined as primary currency.' % unicode(primary_currency))
    ...
    def get_primary(cls):
        ...
        try:
            ...
            raise ImproperlyConfigured(
                'Primary currency not defined in the system.')
...
class Tax(models.Model):
    ...
    def _calculate(cls, price, quantity, method, rate, name=None):
        ...
        if name:
            raise Exception(
                'Unknown tax method "%s" defined for tax rate: "%s"' % (method, name))
